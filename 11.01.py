...
def nauka():
    ...
    if temp_row_algorytm == []:
        print("pusto")
        dane_do_wprowadzenia = [primary_key_slowka, None, None, None, None, None]
    ...
    if wpisane == a[c]:
        ...
        #dane_do_wprowadzenia = temp_row_algorytm
        dane_do_wprowadzenia = [primary_key_slowka, None, None, 0, None, None]
        #dane_do_wprowadzenia[6] = primary_key_slowka
        update_bazy_algorytm(dane_do_wprowadzenia, primary_key_slowka)
        print()
        nauka()
...
def update_bazy_algorytm(algorytm,ids):
    database = r"C:\sqlite\db\pythonsqlite211.db"
    conn = create_connection(database)
    alg = algorytm
    idsl = ids
    cur = conn.cursor()
    cur.execute("UPDATE algorytm SET prawidlowo=?,nieprawidlowo=?, ile_poprawnych_z_rzedu=?,nauczone=?,data_nauczenia=? WHERE id_slowka = ?", (alg[1],alg[2],alg[3],alg[4],alg[5],idsl))
    conn.commit()
    conn.close()
    return
